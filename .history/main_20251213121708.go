...
func SeedOfExercise(c *gin.Context, db *gorm.DB) {
	seedOfExercise := []Exercise{
		{Name: "Push Ups", Description: "Do Push Ups", Category: "Strength", MuscleGroup: "Arm"},
		{Name: "Bench Press", Description: "Do Bench Press", Category: "Strength", MuscleGroup: "Chest"},
		{Name: "Pull Ups", Description: "Do Pull Ups", Category: "Strength", MuscleGroup: "Arm"},
	   }

	   for _, exercises := range seedOfExercise {
	   	err := db.Save(&exercises).Error
		if err != nil {
			fmt.Printf("Error when create Exercises: %s\n", exercises.Name)
		} else {
