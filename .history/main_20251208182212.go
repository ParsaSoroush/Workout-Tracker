...
var db *gorm.DB
var JwtSecretKey = []byte("SECRET_KEY") // replace with env var in production
...
type Exercise struct {
	...
	Category    string    `gorm:"not null" json:"category"`    // e.g., cardio, strength
	MuscleGroup string    `gorm:"not null" json:"muscle_group"` // e.g., chest, back, legs
	CreatedAt   time.Time `json:"created_at"`
}
...
type WorkoutExercise struct {
	...
	Weight      float64  `json:"weight"` // allow zero when not used
...
func SeedExercises() {
	...
	if err := db.Model(&Exercise{}).Count(&count).Error; err != nil {
		log.Println("seed: failed count check:", err)
		return
	}
	if count > 0 {
		log.Println("✅ Exercises already seeded")
		return
	}

	exercises := []Exercise{
		{Name: "Push Up", Description: "Bodyweight chest exercise", Category: "strength", MuscleGroup: "chest"},
		{Name: "Squat", Description: "Lower body compound movement", Category: "strength", MuscleGroup: "legs"},
		{Name: "Plank", Description: "Isometric core exercise", Category: "strength", MuscleGroup: "abs"},
		{Name: "Jump Rope", Description: "Cardio conditioning", Category: "cardio", MuscleGroup: "full body"},
		{Name: "Pull Up", Description: "Upper body pulling", Category: "strength", MuscleGroup: "back"},
		{Name: "Lunge", Description: "Single-leg strength", Category: "strength", MuscleGroup: "legs"},
	}

	if err := db.Create(&exercises).Error; err != nil {
		log.Println("seed: failed to create exercises:", err)
		return
	}
	log.Println("✅ Exercise catalog seeded")
}
...
// Validate JWT and return claims (error if invalid or expired)
...
// Auth middleware attaches claims to context (key: "claims")
...
// Logout: stateless — tell client to drop token
...
// Admin/Seeder endpoint (optional): populate more exercises if needed
// not exposed in routes by default
...
func AddWorkout(c *gin.Context) {
	...
	for _, e := range input.Exercises {
		...
		if err := db.First(&ex, e.ExerciseID).Error; err != nil {
			// rollback created workout and return error
			db.Delete(&workout)
	...
	// preload exercises for response
...
// List upcoming/active workouts for the authenticated user sorted by scheduled_for ASC
...
// Update workout metadata (comments, reschedule)
func UpdateWorkout(c *gin.Context) {
	...
	// respond with updated workout (with exercises)
	if err := db.Preload("Exercises.Exercise").First(&workout, workout.ID).Error; err != nil {
...
// -----------------------------
// WorkoutExercise endpoints (manage exercises inside a workout)
// -----------------------------
...
func UpdateWorkoutExercise(c *gin.Context) {
	...
	exerciseRelID := c.Param("exercise_id") // refers to WorkoutExercise.ID
...
// -----------------------------
// Token check endpoint (returns remaining time + user info)
// -----------------------------
...
func main() {
	...
	r.POST("/logout", Logout) // stateless

	// Token check (public but reads token)
	r.POST("/check", CheckToken)

	// Exercise catalog (public read)
	...
	// Manage workout's exercises (WorkoutExercise)
